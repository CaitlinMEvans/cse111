...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for symbol, quantity in symbol_quantity_list:
        # Get the atomic mass from the periodic table
        name, atomic_mass = periodic_table_dict[symbol]
        # Add the weighted atomic mass to the total
        total_molar_mass += atomic_mass * quantity
    return total_molar_mass


def main():
    """Main function for chemistry program."""
    # Get inputs from the user
    chemical_formula = input("Enter the chemical formula for the molecule: ")
    mass = float(input("Enter the mass of the chemical sample (grams): "))

    # Generate the periodic table
    periodic_table_dict = make_periodic_table()

    # Parse the formula to get the symbol-quantity list
